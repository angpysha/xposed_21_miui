...
/data/busybox//data/busybox/find $HELPERPATH/system -type f -name "*.odex*.xposed" -size 0 2>/dev/null | while read ODEX; do
  ORIG=${ODEX%.xposed}
...
/data/busybox//data/busybox/find $HELPERPATH/system/framework -type f -name "boot.*" -size 0 2>/dev/null | while read BOOT; do
